...
scp $PUB_KEY $ROOT_USERNAME@$ESXI_HOST:$REMOTE_TMP_PUB_KEY
ssh $ROOT_USERNAME@$ESXI_HOST "mkdir -p /etc/ssh/keys-$USERNAME && cat $REMOTE_TMP_PUB_KEY >> /etc/ssh/keys-$USERNAME/authorized_keys && rm $REMOTE_TMP_PUB_KEY"
